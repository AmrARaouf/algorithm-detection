...
void solve() {
	cin>>n>>m;
	while (m--) {int x, y; cin>>x>>y; x--; y--; adj[x].push_back(y); adj[y].push_back(x); mp[ii(x, y)]++; mp[ii(y, x)]++;}
	vi d;
	FOR(i, 0, n) {if (sz(adj[i]) % 2) d.push_back(i);}
	//if (sz(d) % 2) {int i = d.back(); d.pop_back(); adj[i].push_back(i); adj[i].push_back(i); mp[ii(i, i)] += 2;}
