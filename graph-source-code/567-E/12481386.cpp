...
struct edge{
    ...
    void out(){
        if (Ds[a]+w+Dt[b]==Ds[t] && Fs[a]*Ft[b]==Fs[t]) puts("YES");
        else{
            LL d = (Ds[a]+w+Dt[b]) - (Ds[t]-1);
            if (d < w) printf("CAN %d\n", d);
            else puts("NO");
        }
    }
...
void relax(LL d[], Int f[], int u, int v, int w){
    if (d[v] > d[u] + w){
        d[v] = d[u] + w;
        Q.push(MP(d[v], v));
        f[v] = f[u];
...
void Dijkstra(int s, LL d[], Int f[], VII adj[]){
    CLR(Q); fill(d+1, d+n+1, INFF); fill(f+1, f+n+1, 0);
    d[s] = 0; f[s] = 1; Q.push(MP(0, s));
    while (!Q.empty()){
...
int main() {
    ...
    RD(n, m, s, t); REP(i, m) E[i].in();

    Dijkstra(s, Ds, Fs, adj);
    Dijkstra(t, Dt, Ft, radj);
