...
int rrr[]={1,1,0,-1,-1,-1,0,1,0};int ccc[]={0,1,1,1,0,-1,-1,-1,0};  //8 direction
...
bool dfs(int x,int y,int move)
{
    ...
    for(int i=0;i<9;i++)
    {
        ...
        {
            if(r>=move and grid[r-move][c]=='S') continue;
            if(r>=move+1 and grid[r-move-1][c]=='S') continue;

            if(dfs(r,c,move+1)) return true;
        }
    }
    return false;
}
