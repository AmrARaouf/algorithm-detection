...
int ma[120][120];
int main(){
    for(int i=3;i<=100;i++) sol[i]=i*(i-1)*(i-2)/6;
    int k,l,t;
    cin>>k;
    for(int i=0;sol[i]<=k;i++) l=i;
    for(int i=1;i<=l;i++) res[i]=i;
    k-=sol[l];
    for(int i=1;i<=l;i++)
        for(int j=1;j<=l;j++)
            if(i!=j) ma[i][j]=1;
    while(k){
        l++;
        for(int i=2;i*(i-1)/2<=k;i++) res[l]=i;
        k-=res[l]*(res[l]-1)/2;
        for(int i=1;i<=res[l];i++) ma[i][l]=ma[l][i]=1;
    }
    printf("%d\n",l);
    for(int i=1;i<=l;i++){
        for(int j=1;j<=l;j++) printf("%d",ma[i][j]);
        putchar('\n');
    }
    return 0;
}
